...
C c;			默认构造函数，构造空容器
C cl(c2);		构造c2的拷贝c1
C c(b，e); 构造c，将迭代器b和e指定的范围内的元素拷贝到c(array不支持) C
c{a,bc...};	列表初始化c
...
a.swap(b)
交换a和b的元素a和b必须具有相同的类型。swap通常比从a向b拷贝元素快得多 swap(a，b)
与a.swap(b)等价
...
注：赋值相关运算会导致指向左边容器内部的迭代器、引用和指针失效。而swap
操作将容器内容交换不会导致指向容器的迭代器、引用和指针失效(容器类型为 array 和
string 的情况除外)。
...
forward_list不支持push_back和emplace_back。 vector和string
不支持push_front和emplace_front。 cpush back(t)
在c的尾部创建一个值为t或由args创建的元素。返回void c.emplace back(args) cpush front(t)	在c的头部创建一个值为t或由args创建的元素。返回void cemplace
front(args) c.insert(p，t)
...
cinsert(p,il)
i1是一个花括号包围的元素值列表。将这些给定值插入到迭代器p指向的元素之前。返回指向新添加的第一个元素的迭代器:若列表为空，则返回p
...
c[n]
返回c中下标为n的元素的引用n是一个无符号整数。若n>=csize()，则函数行为未定义
...
il是一个花括号列表。返回一个指向最后一个插入元素的迭代器。如果范围为空，则返回p。若p为尾后迭代器，则函数行为未定义
emplace_after(p,args)
使用args在p指定的位置之后创建一个元素。返回一个指向这个新元素的迭代器。若p为尾后迭代器，则函数行为未定义
lst.erase_after(p)
删除p指向的位置之后的元素，或删除从b之后直到(但不包含)e之间的元素。返回一个指向被删元素之后元素的迭代
器，若不存在这样的元素，则返回尾后迭代器。如果p指向
lst的尾元素或者是一个尾后迭代器，则函数行为未定义 lsteraseafter(b,e)
*/
/*
容器大小管理操作
...
int main() {
  ...
  //	容器的比较——关系运算符
  cout << (v1 < v2)
       << endl; // true		v1和v2 在元素[2]处不同：v1[2]小于等于v2[2]
